...
export default function OAuthButtons({ next }: { next?: string }) {
  const target = next || '/account/dashboard'
  const redirectTo =
  ...
  return (
    <div className="account-social-grid">
      {/* Google */}
      <button type="button" className="account-social-btn" onClick={() => handleOAuth('google')}>
      ...
      {/* Apple */}
      <button type="button" className="account-social-btn" onClick={() => handleOAuth('apple')}>
      ...
      <button
        type="button"
        className="account-social-btn account-social-btn--line"
      ...
      >
        ...
        Continue with LINE
        <span className="account-social-soon">Coming soon</span>
      </button>
    </div>
  )
}
